...
# Say we'd like to obtain mesh elements with size lc/30 near curve 2 and
# point 5, and size lc elsewhere. To achieve this, we can use two fields:
# "Distance" and "Threshold". We first define a Distance field ('Field[1]')
# on point 5 and on curve 2. This field returns the distance to point 5 and
...
# Define 'Threshold' field, which uses the return value of the 'Distance'
# field 1 in order to define a simple change in element size depending on
...
# of the spatial coordinates. We can do this with MathEval field.
...
# cubic law, with minimum element size = lc/100).
gmsh.model.mesh.field.add('MathEval', 5)
gmsh.model.mesh.field.setString(5, 'F', 'F4^3 + ' + str(lc/100))

# We could also use a 'Box' field to impose a step change in element sizes inside a 
# box.
